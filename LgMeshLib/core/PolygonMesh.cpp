...
PolygonMesh::Vertex
PolygonMesh::add_vertex(const Vec3& p)
{
  Vertex v = new_vertex();
  vpoint_[v] = p;
  return v;
...
bool PolygonMesh::read(const std::string& filename)
{
  return read_poly(*this, filename);
}



bool PolygonMesh::write(const std::string& filename) const
{
  // TODO
  //return write_poly(*this, filename);
  return true;
}
...
Scalar PolygonMesh::compute_laplacian_cot(Edge e) const
{
  ...
  if (!is_boundary(halfedge(e, 0)))
  {
    ...
    alpha_cos = fabs((e3 * e3 + e2 * e2 - e1 * e1) / (2 * e3 * e2));
  }
  if (!is_boundary(halfedge(e, 1)))
  {
    ...
    Scalar e2 = (v1 - v2).norm();
    Scalar e3 = (v0 - v2).norm();
    beta_cos = fabs((e3 * e3 + e2 * e2 - e1 * e1) / (2 * e3 * e2));
  }
